create proc EnrollmentStatus_get as
...
LEFT OUTER JOIN (SELECT st.[StudentColleagueID], t.[TermStartDate], FirstNewEnrollmentDate, t.Term, max(cast(scs.[HasMaintainedEnrollment] as int)) HasMaintainedEnrollment
			   ...
			   left join AARDW.AARPL.[StudentCourseSection] [scs] ON scs.[StudentTerm_SK] = st.[StudentTerm_SK]
			   left JOIN (SELECT st.[StudentColleagueID], MIN(t.[TermStartDate]) AS FirstNewEnrollmentDate
						  FROM AARDW.AARPL.StudentTerm AS st
						  JOIN AARDW.AARPL.Term t ON t.[Term_SK] = st.[Term_SK]
						  JOIN AARDW.AARPL.[StudentCourseSection] AS [scs] ON scs.[StudentTerm_SK] = st.[StudentTerm_SK]
