if exists (select 1 from sys.objects where object_id = object_id('GoalCompletionRate_get'))
   set noexec on
go
create procedure dbo.[GoalCompletionRate_get] as
begin
...
GRANT EXECUTE ON GoalCompletionRate_get TO PUBLIC;

GO

alter proc [dbo].[GoalCompletionRate_get] as
